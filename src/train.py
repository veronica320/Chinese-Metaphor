...
def train_model(gpu, input_num, nb_class, rel_dic, trainfile, valfile, model_path, model_name, epoch):
    ...
    model = multi_cgru_keras.creat_model(gpu, model_name, mode, nb_class, nb_filter, wsize, maxlen, cvsize, wvsize,
                                         denseuni, cnn_dropout, l2, lr, w2v, w2vdim, c2vmat)
...
def plot_training(history, name):
    ...
    val_fscore = history.history['val_fscore']
    loss = history.history['loss']
    val_loss = history.history['val_loss']
    epochs = [i + 1 for i in range(len(fscore))]
    with PdfPages(name + '.pdf') as pdf:
        ...
        plt.subplot(212)
        plt.plot(epochs, fscore, 'r:', label='train_fscore')
        plt.plot(epochs, val_fscore, 'b:', label='val_fscore')
        plt.xlabel('epoch')
        plt.ylabel('fscore')
